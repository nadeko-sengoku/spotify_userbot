...
async def _(event):
    ...
    async with aiohttp.ClientSession() as session:
        response_api_zero = await session.get(sample_url.format(input_str, OPEN_WEATHER_MAP_APPID))
...
@client.on(events.NewMessage(outgoing=True, pattern=CMD_PREFIX + "wttr (.*)"))
async def _(event):
    if event.fwd_from:
        return
