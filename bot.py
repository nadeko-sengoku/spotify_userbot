...
if Config.CONSOLE_LOGGER_VERBOSE:
    basicConfig(
...
with TelegramClient(StringSession(Config.SESSION_KEY), Config.API_ID, Config.API_HASH, device_model=device_model,
...
if Config.SPOTIFY_MODE:
	...
	async def work():
		while True:
			...
			# 401 means our access token is expired, so we need to refresh it
				...
				data = {"Config.CLIENT_ID": Config.CLIENT_ID, "Config.CLIENT_SECRET": Config.CLIENT_SECRET,
			...
			try:
				...
				if to_insert:
					...
					# we need this variable to see if actually one of the Config.BIOS is below the character limit
					new_bio = ""
					for bio in Config.BIOS:
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "getdeez"))
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "getsong"))
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "song"))
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "alive"))
	...
	**Recently played songs:** \n{recent_p}"""
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "me"))
	async def sme(event):
		oauth = {
				"Authorization": "Bearer " + database.return_token()}
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "recent"))
	...
	@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "bio"))
    ...
    LOGGER.info("Spotify_mode: False")
# =========================== LOAD PLUGINS ============================================#     
...
@client.on(events.NewMessage(outgoing=True, pattern=Config.CMD_PREFIX + "restart"))
async def shutdown_handler(event):
	logger.error("SHUT DOWN")
	CMD = Config.CMD_PREFIX[1:]
